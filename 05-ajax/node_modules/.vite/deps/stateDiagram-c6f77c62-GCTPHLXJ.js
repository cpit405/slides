...
// node_modules/mermaid/dist/stateDiagram-c6f77c62.js
var L2 = {};
var $ = (e, i) => {
  L2[e] = i;
};
var v = (e) => L2[e];
var P = () => Object.keys(L2);
var I = () => P().length;
var _ = {
  get: v,
  set: $,
...
var K = (e, i) => {
  const o = function(l, m, w2) {
    const k = l.append("tspan").attr("x", 2 * Ft().state.padding).text(m);
    w2 || k.attr("dy", Ft().state.textHeight);
...
var Q = (e, i, o) => {
  ...
  const w2 = 1 - Ft().state.textHeight;
  return e.insert("rect", ":first-child").attr("x", l).attr("y", w2).attr("class", o ? "alt-composit" : "composit").attr("width", c).attr(
    "height",
...
var tt = (e, i, o, d) => {
  ...
  let p = e.replace(/\r\n/g, "<br/>");
  p = p.replace(/\n/g, "<br/>");
  const a = p.split(Rn.lineBreakRegex);
...
var at = function(e, i, o) {
  ...
  const s = i.points, g = S().x(function(r) {
    ...
    return r.y;
  }).curve(Uf), p = e.append("path").attr("d", g(s)).attr("id", "edge" + A).attr("class", "transition");
  let a = "";
  if (Ft().state.arrowMarkerAbsolute && (a = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.search, a = a.replace(/\(/g, "\\("), a = a.replace(/\)/g, "\\)")), p.attr(
    ...
    const r = e.append("g").attr("class", "stateLabel"), { x: y, y: x } = Di.calcLabelPosition(i.points), c = Rn.getRows(o.title);
    let l = 0;
    const m = [];
    let w2 = 0, k = 0;
    for (let u = 0; u <= c.length; u++) {
      const h = r.append("text").attr("text-anchor", "middle").text(c[u]).attr("x", y).attr("y", x + l), f = h.node().getBBox();
      w2 = Math.max(w2, f.width), k = Math.min(k, f.x), w.info(f.x, y, x + l), l === 0 && (l = h.node().getBBox().height, w.info("Title height", l, x)), m.push(h);
    }
    let E = l * c.length;
    if (c.length > 1) {
    ...
    r.insert("rect", ":first-child").attr("class", "box").attr("x", y - w2 / 2 - Ft().state.padding / 2).attr("y", x - E / 2 - Ft().state.padding / 2 - 3.5).attr("width", w2 + Ft().state.padding).attr("height", E + Ft().state.padding), w.info(n);
  }
  A++;
};
var B;
...
var st = function(e, i, o, d) {
  ...
  w.debug("Rendering diagram " + e);
  const r = p.select(`[id='${i}']`);
  nt(r);
  const y = d.db.getRootDoc();
  W(y, r, void 0, false, p, a, d);
  const x = B.padding, c = r.node().getBBox(), l = c.width + x * 2, m = c.height + x * 2, w2 = l * 1.75;
  Tl(r, m, w2, B.useMaxWidth), r.attr(
    "viewBox",
...
var W = (e, i, o, d, s, g, p) => {
  ...
  w.debug("Count=", a.nodeCount(), a);
  let m = 0;
  c.forEach(function(n) {
    m++, w.debug("Setting edge", n), a.setEdge(
      n.id1,
      n.id2,
      {
        relation: n,
        width: rt(n.title),
        height: B.labelHeight * Rn.getRows(n.title).length,
        labelpos: "c"
      },
      "id" + m
    );
  }), js(a), w.debug("Graph after layout", a.nodes());
  const w2 = i.node();
  a.nodes().forEach(function(n) {
    n !== void 0 && a.node(n) !== void 0 ? (w.warn("Node " + n + ": " + JSON.stringify(a.node(n))), s.select("#" + w2.id + " #" + n).attr(
      "transform",
      "translate(" + (a.node(n).x - a.node(n).width / 2) + "," + (a.node(n).y + (z[n] ? z[n].y : 0) - a.node(n).height / 2) + " )"
    ), s.select("#" + w2.id + " #" + n).attr("data-x-shift", a.node(n).x - a.node(n).width / 2), g.querySelectorAll("#" + w2.id + " #" + n + " .divider").forEach((h) => {
    ...
    })) : w.debug("No Node " + n + ": " + JSON.stringify(a.node(n)));
  });
  let k = w2.getBBox();
  a.edges().forEach(function(n) {
    n !== void 0 && a.edge(n) !== void 0 && (w.debug("Edge " + n.v + " -> " + n.w + ": " + JSON.stringify(a.edge(n))), at(i, a.edge(n), a.edge(n).relation));
  }), k = w2.getBBox();
  const E = {
    id: o || "root",
    label: o || "root",
  ...
  return E.width = k.width + 2 * B.padding, E.height = k.height + 2 * B.padding, w.debug("Doc rendered", E, a), E;
};
var ot = {
  setConf: it,
  draw: st
...
//# sourceMappingURL=stateDiagram-c6f77c62-GCTPHLXJ.js.map
