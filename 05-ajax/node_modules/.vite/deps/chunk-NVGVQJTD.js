...
// node_modules/mermaid/dist/edges-6be02a86.js
var nt = (r, t, e, n) => {
  t.forEach((a) => {
    xt[a](r, e, n);
  });
...
var wt = (r, t, e, n) => {
  let a = r || "";
  if (typeof a == "object" && (a = a[0]), Ta(Ft().flowchart.htmlLabels)) {
    a = a.replace(/\\n|\n/g, "<br />"), w.info("vertexText" + a);
    const s = {
      isNode: n,
      label: Ab(a).replace(
        /fa[blrs]?:fa-[\w-]+/g,
...
var B = async (r, t, e, n) => {
  let a;
  const s = t.useHtmlLabels || Ta(Ft().flowchart.htmlLabels);
  ...
  const o = l.node();
  let h;
  t.labelType === "markdown" ? h = Rr(l, ti(Ab(c), Ft()), {
    useHtmlLabels: s,
...
var G = async (r, t) => {
  ...
  w.info("Question main (Circle)");
  const c = _(e, i, i, l);
  return c.attr("style", t.style), v(t, c), t.intersect = function(o) {
    return w.warn("Intersect called"), m.polygon(t, l, o);
  }, e;
};
var Ct2 = (r, t) => {
...
var Xt = async (r, t) => {
  ...
  if (s.attr("class", "basic label-container").attr("style", t.style).attr("rx", t.rx).attr("ry", t.ry).attr("x", -n.width / 2 - a).attr("y", -n.height / 2 - a).attr("width", i).attr("height", l), t.props) {
    ...
    t.props.borders && (et(s, t.props.borders, i, l), c.delete("borders")), c.forEach((o) => {
      w.warn(`Unknown node property ${o}`);
    });
  }
...
var Yt = async (r, t) => {
  ...
  if (n.attr("width", a).attr("height", s), e.attr("class", "label edgeLabel"), t.props) {
    ...
    t.props.borders && (et(n, t.props.borders, a, s), i.delete("borders")), i.forEach((l) => {
      w.warn(`Unknown node property ${l}`);
    });
  }
...
function et(r, t, e, n) {
  ...
  t.includes("t") ? (w.debug("add top border"), s(e)) : i(e), t.includes("r") ? (w.debug("add right border"), s(n)) : i(n), t.includes("b") ? (w.debug("add bottom border"), s(e)) : i(e), t.includes("l") ? (w.debug("add left border"), s(n)) : i(n), r.attr("stroke-dasharray", a.join(" "));
}
var At = (r, t) => {
  let e;
  ...
  let c = "";
  typeof l == "object" ? c = l[0] : c = l, w.info("Label text abc79", c, l, typeof l == "object");
...
var jt = async (r, t) => {
  ...
  return s.attr("style", t.style).attr("rx", t.rx).attr("ry", t.ry).attr("r", n.width / 2 + a).attr("width", n.width + t.padding).attr("height", n.height + t.padding), w.info("Circle main"), v(t, s), t.intersect = function(i) {
    return w.info("Circle intersect", t, n.width / 2 + a, i), m.circle(t, n.width / 2 + a, i);
  }, e;
};
var Dt = async (r, t) => {
  const { shapeSvg: e, bbox: n, halfPadding: a } = await B(
...
var Ft2 = (r, t) => {
  ...
  ), d += M2.height + n, c.attr("class", "divider").attr("x1", -h / 2 - e).attr("x2", h / 2 + e).attr("y1", -f / 2 - e + a + d).attr("y2", -f / 2 - e + a + d), d += a, W.forEach((k) => {
    Ct(k).attr(
      "transform",
...
var ir = (r, t) => {
  const e = Ta(Ft().flowchart.htmlLabels), n = t.labelType === "markdown" ? Rr(r, t.label, {
    style: t.labelStyle,
    useHtmlLabels: e,
    addSvgBackground: true
  }) : I(t.label, t.labelStyle);
  w.info("abc82", t, t.labelType);
  ...
  if (t.startLabelLeft) {
    ...
    const f = c.getBBox();
    h.attr("transform", "translate(" + -f.width / 2 + ", " + -f.height / 2 + ")"), S2[t.id] || (S2[t.id] = {}), S2[t.id].startLeft = o, j(l, t.startLabelLeft);
  }
  if (t.startLabelRight) {
    ...
    const f = c.getBBox();
    h.attr("transform", "translate(" + -f.width / 2 + ", " + -f.height / 2 + ")"), S2[t.id] || (S2[t.id] = {}), S2[t.id].startRight = o, j(l, t.startLabelRight);
  }
  if (t.endLabelLeft) {
    const c = I(t.endLabelLeft, t.labelStyle), o = r.insert("g").attr("class", "edgeTerminals"), h = o.insert("g").attr("class", "inner");
    l = h.node().appendChild(c);
    const f = c.getBBox();
    h.attr("transform", "translate(" + -f.width / 2 + ", " + -f.height / 2 + ")"), o.node().appendChild(c), S2[t.id] || (S2[t.id] = {}), S2[t.id].endLeft = o, j(l, t.endLabelLeft);
  }
  if (t.endLabelRight) {
    const c = I(t.endLabelRight, t.labelStyle), o = r.insert("g").attr("class", "edgeTerminals"), h = o.insert("g").attr("class", "inner");
    l = h.node().appendChild(c);
    const f = c.getBBox();
    h.attr("transform", "translate(" + -f.width / 2 + ", " + -f.height / 2 + ")"), o.node().appendChild(c), S2[t.id] || (S2[t.id] = {}), S2[t.id].endRight = o, j(l, t.endLabelRight);
  }
  return n;
};
function j(r, t) {
...
var nr = (r, t) => {
  w.info("Moving label abc78 ", r.id, r.label, D[r.id]);
...
var Vt = (r, t, e) => {
  w.warn(`intersection calc abc89:
  ...
  if (Math.abs(a - t.y) * i > Math.abs(n - t.x) * c) {
    ...
    return l === 0 && (u.x = t.x, u.y = t.y), h === 0 && (u.x = t.x), o === 0 && (u.y = t.y), w.warn(`abc89 topp/bott calc, Q ${o}, q ${f}, R ${h}, r ${l}`, u), u;
  } else {
    e.x < t.x ? l = t.x - i - n : l = n - i - t.x;
    let f = o * l / h, u = e.x < t.x ? e.x + h - l : e.x - h + l, d = e.y < t.y ? e.y + f : e.y - f;
    return w.warn(`sides calc abc89, Q ${o}, q ${f}, R ${h}, r ${l}`, { _x: u, _y: d }), l === 0 && (u = t.x, d = t.y), h === 0 && (u = t.x), o === 0 && (d = t.y), { x: u, y: d };
  }
};
var tt = (r, t) => {
  w.warn("abc88 cutPathAtIntersect", r, t);
  let e = [], n = r[0], a = false;
  return r.forEach((s) => {
    if (w.info("abc88 checking point", s, t), !Qt(t, s) && !a) {
      const i = Vt(t, n, s);
      w.warn("abc88 inside", s, n, i), w.warn("abc88 intersection", i);
      let l = false;
      e.forEach((c) => {
        l = l || c.x === i.x && c.y === i.y;
      }), e.some((c) => c.x === i.x && c.y === i.y) ? w.warn("abc88 no intersect", i, e) : e.push(i), a = true;
    } else
      w.warn("abc88 outside", s, n), n = s, a || e.push(s);
  }), w.warn("abc88 returning points", e), e;
};
var lr = function(r, t, e, n, a, s) {
  let i = e.points, l = false;
  const c = s.node(t.v);
  var o = s.node(t.w);
  w.info("abc88 InsertEdge: ", e), o.intersect && c.intersect && (i = i.slice(1, e.points.length - 1), i.unshift(c.intersect(i[0])), w.info(
    "Last point",
    i[i.length - 1],
    o,
    o.intersect(i[i.length - 1])
  ), i.push(o.intersect(i[i.length - 1]))), e.toCluster && (w.info("to cluster abc88", n[e.toCluster]), i = tt(e.points, n[e.toCluster].node), l = true), e.fromCluster && (w.info("from cluster abc88", n[e.fromCluster]), i = tt(i.reverse(), n[e.fromCluster].node).reverse(), l = true);
  const h = i.filter((b) => !Number.isNaN(b.y));
  let f;
  a === "graph" || a === "flowchart" ? f = e.curve || Uf : f = Uf;
  ...
  let y = "";
  switch ((Ft().flowchart.arrowMarkerAbsolute || Ft().state.arrowMarkerAbsolute) && (y = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.search, y = y.replace(/\(/g, "\\("), y = y.replace(/\)/g, "\\)")), w.info("arrowTypeStart", e.arrowTypeStart), w.info("arrowTypeEnd", e.arrowTypeEnd), e.arrowTypeStart) {
    case "arrow_cross":
...
//# sourceMappingURL=chunk-NVGVQJTD.js.map
