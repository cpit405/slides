import {
  ...
  we,
  ye
} from "./chunk-AR3GG3SZ.js";
import "./chunk-POYP5CRU.js";
import {
  Jt,
  Qt
} from "./chunk-WGAOAKKX.js";
import "./chunk-NVGVQJTD.js";
import "./chunk-KQ24I337.js";
import "./chunk-QHXPNRNQ.js";
import {
  D,
  L,
  Un,
...
// node_modules/mermaid/dist/flowDiagram-5d2b0cd4.js
function Dt(r) {
  if (!r.ok)
...
function Wt(r, e, t, n) {
  ...
  te(s, t[n + "Style"]), t[n + "Class"] && s.attr("class", t[n + "Class"]);
}
function $t(r, e, t, n) {
...
function Zt(r, e) {
  ...
  return t.append("path").attr("class", "path").attr("d", function(n) {
    var a = e.edge(n), s = e.node(n.v).elem, i = D(a.points.length).map(function() {
      return Kt(s);
    });
    return ot(a, i);
  }), t.append("defs"), t;
...
var q = function(r, e, t) {
  ...
  a.exit().remove(), a.enter().append("g").attr("class", "node").style("opacity", 0), a = r.selectAll("g.node"), a.each(function(i) {
    ...
    var d = c2.append("g").attr("class", "label"), l = K(d, o), v = t[o.shape], h = un(l.node().getBBox(), "width", "height");
...
function ne(r, e, t, n) {
  var a, s, i, o, c2, d, l, v, h, u, p, g, f, y, k;
  if (a = e.y - r.y, i = r.x - e.x, c2 = e.x * r.y - r.x * e.y, h = a * t.x + i * t.y + c2, u = a * n.x + i * n.y + c2, !(h !== 0 && u !== 0 && j(h, u)) && (s = n.y - t.y, o = t.x - n.x, d = n.x * t.y - t.x * n.y, l = s * r.x + o * r.y + d, v = s * e.x + o * e.y + d, !(l !== 0 && v !== 0 && j(l, v)) && (p = a * o - s * i, p !== 0)))
    return g = Math.abs(p / 2), f = i * d - o * c2, y = f < 0 ? (f - g) / p : (f + g) / p, f = s * c2 - a * d, k = f < 0 ? (f - g) / p : (f + g) / p, { x: y, y: k };
}
function j(r, e) {
  return r * e > 0;
}
...
function de() {
  var r = function(e, t) {
    fe(t);
    var n = D2(e, "output"), a = D2(n, "clusters"), s = D2(n, "edgePaths"), i = X(D2(n, "edgeLabels"), t), o = q(D2(n, "nodes"), t, Q);
    js(t), re2(o, t), ee(i, t), F(s, t, Y2);
    var c2 = H(a, t);
    te2(c2, t), pe2(t);
  };
...
function fe(r) {
  r.nodes().forEach(function(e) {
    var t = r.node(e);
    !b(t, "label") && !r.children(e).length && (t.label = e), b(t, "paddingX") && va(t, {
    ...
    }), va(t, he2), c(["paddingLeft", "paddingRight", "paddingTop", "paddingBottom"], function(n) {
      t[n] = Number(t[n]);
    }), b(t, "width") && (t._prevWidth = t.width), b(t, "height") && (t._prevHeight = t.height);
  }), r.edges().forEach(function(e) {
    var t = r.edge(e);
    b(t, "label") || (t.label = ""), va(t, ue2);
  });
}
function pe2(r) {
...
var bt = function(r, e, t, n, a, s) {
  ...
  Object.keys(r).forEach(function(d) {
    ...
    if (Ta(Ft().flowchart.htmlLabels)) {
      ...
      const k = u.split(Rn.lineBreakRegex);
      for (const I of k) {
    ...
    w.warn("Adding node", l.id, l.domId), e.setNode(s.db.lookUpDomId(l.id), {
      labelType: "svg",
...
var kt = function(r, e, t) {
  ...
  r.forEach(function(i) {
    ...
    l.style = v, l.labelStyle = h, i.interpolate !== void 0 ? l.curve = bm(i.interpolate, Wf) : r.defaultInterpolate !== void 0 ? l.curve = bm(r.defaultInterpolate, Wf) : l.curve = bm(xt.curve, Wf), i.text === void 0 ? i.style !== void 0 && (l.arrowheadStyle = "fill: #333") : (l.arrowheadStyle = "fill: #333", l.labelpos = "c", Ta(Ft().flowchart.htmlLabels) ? (l.labelType = "html", l.label = `<span id="L-${o}" class="edgeLabel L-${c2}' L-${d}" style="${l.labelStyle}">${i.text.replace(
      /fa[blrs]?:fa-[\w-]+/g,
      (u) => `<i class='${u.replace(":", " ")}'></i>`
    )}</span>`) : (l.labelType = "text", l.label = i.text.replace(Rn.lineBreakRegex, `
...
var me = function(r, e) {
  return w.info("Extracting classes"), e.db.getClasses();
};
var xe = function(r, e, t, n) {
  ...
  d === void 0 && (d = "TD");
  const l = s.nodeSpacing || 50, v = s.rankSpacing || 50, h = new L({
    multigraph: true,
    compound: true
  }).setGraph({
    rankdir: d,
  ...
  for (let w2 = p.length - 1; w2 >= 0; w2--)
    u = p[w2], n.db.addVertex(u.id, u.title, "group", void 0, u.classes);
  const g = n.db.getVertices();
  w.warn("Get vertices", g);
  const f = n.db.getEdges();
  let y = 0;
  for (y = p.length - 1; y >= 0; y--) {
    u = p[y], Y("cluster").append("text");
    for (let w2 = 0; w2 < u.nodes.length; w2++)
      w.warn(
        "Setting subgraph",
        u.nodes[w2],
        n.db.lookUpDomId(u.nodes[w2]),
        n.db.lookUpDomId(u.id)
      ), h.setParent(n.db.lookUpDomId(u.nodes[w2]), n.db.lookUpDomId(u.id));
  }
  bt(g, h, e, o, c2, n), kt(f, h, n);
  const k = new de();
  ge.addToRender(k), k.arrows().none = function(b2, L2, m, B) {
    const C = b2.append("marker").attr("id", L2).attr("viewBox", "0 0 10 10").attr("refX", 9).attr("refY", 5).attr("markerUnits", "strokeWidth").attr("markerWidth", 8).attr("markerHeight", 6).attr("orient", "auto").append("path").attr("d", "M 0 0 L 0 0 L 0 0 z");
    te(C, m[B + "Style"]);
  }, k.arrows().normal = function(b2, L2) {
    b2.append("marker").attr("id", L2).attr("viewBox", "0 0 10 10").attr("refX", 9).attr("refY", 5).attr("markerUnits", "strokeWidth").attr("markerWidth", 8).attr("markerHeight", 6).attr("orient", "auto").append("path").attr("d", "M 0 0 L 10 5 L 0 10 z").attr("class", "arrowheadPath").style("stroke-width", 1).style("stroke-dasharray", "1,0");
  };
  ...
  for (k(E, h), E.selectAll("g.node").attr("title", function() {
    ...
    if (u = p[y], u.title !== "undefined") {
      const w2 = c2.querySelectorAll(
        "#" + e + ' [id="' + n.db.lookUpDomId(u.id) + '"] rect'
      ), b2 = c2.querySelectorAll(
        "#" + e + ' [id="' + n.db.lookUpDomId(u.id) + '"]'
      ), L2 = w2[0].x.baseVal.value, m = w2[0].y.baseVal.value, B = w2[0].width.baseVal.value, C = Ct(b2[0]).select(".label");
      C.attr("transform", `translate(${L2 + B / 2}, ${m + 14})`), C.attr("id", e + "Text");
      for (let W = 0; W < u.classes.length; W++)
        b2[0].classList.add(u.classes[W]);
    }
  if (!s.htmlLabels) {
    const w2 = c2.querySelectorAll('[id="' + e + '"] .edgeLabel .label');
    for (const b2 of w2) {
      const L2 = b2.getBBox(), m = c2.createElementNS("http://www.w3.org/2000/svg", "rect");
      m.setAttribute("rx", 0), m.setAttribute("ry", 0), m.setAttribute("width", L2.width), m.setAttribute("height", L2.height), b2.insertBefore(m, b2.firstChild);
    }
  }
  Cy(h, I, s.diagramPadding, s.useMaxWidth), Object.keys(g).forEach(function(w2) {
    const b2 = g[w2];
    if (b2.link) {
      const L2 = o.select("#" + e + ' [id="' + n.db.lookUpDomId(w2) + '"]');
      if (L2) {
        ...
        const B = L2.insert(function() {
          return m;
        }, ":first-child"), G = L2.select(".label-container");
        G && B.append(function() {
          return G.node();
        });
        const C = L2.select(".label");
        C && B.append(function() {
          return C.node();
        });
      }
...
//# sourceMappingURL=flowDiagram-5d2b0cd4-HAOOTFKT.js.map
