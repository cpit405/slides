import {
  L,
  js
} from "./chunk-SGJYBFGJ.js";
import {
  S
} from "./chunk-RJUWN7YA.js";
import "./chunk-NBSQHQBT.js";
import "./chunk-Y5P3ULHH.js";
import {
  $l,
  Al,
  Ct,
...
// node_modules/mermaid/dist/erDiagram-9aadf2be.js
var ct = function() {
  var t = function(M, a, l, d) {
    for (l = l || {}, d = M.length; d--; l[M[d]] = a)
      ;
    return l;
  }, r = [1, 2], e = [1, 5], n = [6, 9, 11, 23, 25, 27, 29, 30, 31, 52], s = [1, 17], p = [1, 18], f = [1, 19], o = [1, 20], c = [1, 21], _ = [1, 22], E = [1, 25], g = [1, 30], O = [1, 31], y = [1, 32], A = [1, 33], Y = [1, 34], U = [6, 9, 11, 15, 20, 23, 25, 27, 29, 30, 31, 44, 45, 46, 47, 48, 52], X = [1, 46], I = [30, 31, 49, 50], N = [4, 6, 9, 11, 23, 25, 27, 29, 30, 31, 52], v = [44, 45, 46, 47, 48], k = [22, 37], x = [1, 66], P = [1, 65], Z = [22, 37, 39, 41], w2 = {
    trace: function() {
    },
    yy: {},
    ...
    parse: function(a) {
      var l = this, d = [0], u = [], m = [null], i = [], Q = this.table, h = "", J = 0, ht = 0, Et = 2, dt = 1, mt = i.slice.call(arguments, 1), b = Object.create(this.lexer), G = { yy: {} };
      ...
      for (var D, z, L2, at, K = {}, $, F, ut, tt; ; ) {
        if (z = d[d.length - 1], this.defaultActions[z] ? L2 = this.defaultActions[z] : ((D === null || typeof D > "u") && (D = kt()), L2 = Q[z] && Q[z][D]), typeof L2 > "u" || !L2.length || !L2[0]) {
          var nt = "";
          tt = [];
          for ($ in Q[z])
...
Expecting ` + tt.join(", ") + ", got '" + (this.terminals_[D] || D) + "'" : nt = "Parse error on line " + (J + 1) + ": Unexpected " + (D == dt ? "end of input" : "'" + (this.terminals_[D] || D) + "'"), this.parseError(nt, {
        ...
        if (L2[0] instanceof Array && L2.length > 1)
          throw new Error("Parse Error: multiple actions possible at state: " + z + ", token: " + D);
        switch (L2[0]) {
          case 1:
            d.push(D), m.push(b.yytext), i.push(b.yylloc), d.push(L2[1]), D = null, ht = b.yyleng, h = b.yytext, J = b.yylineno, it = b.yylloc;
            break;
          case 2:
            if (F = this.productions_[L2[1]][1], K.$ = m[m.length - F], K._$ = {
              ...
              return at;
            F && (d = d.slice(0, -1 * F * 2), m = m.slice(0, -1 * F), i = i.slice(0, -1 * F)), d.push(this.productions_[L2[1]][0]), m.push(K.$), i.push(K._$), ut = Q[d[d.length - 2]][d[d.length - 1]], d.push(ut);
            break;
          case 3:
            return true;
        }
...
` + this.showPosition(), {
  ...
  return S2.prototype = w2, w2.Parser = S2, new S2();
}();
ct.parser = ct;
var Ct2 = ct;
var j = {};
...
var Zt = function(t, r, e) {
  ge.parseDirective(this, t, r, e);
};
var pt = function(t) {
  return j[t] === void 0 && (j[t] = { attributes: [] }, w.info("Added new entity :", t)), j[t];
};
var Ft2 = () => j;
var Wt = function(t, r) {
  let e = pt(t), n;
  for (n = r.length - 1; n >= 0; n--)
    e.attributes.push(r[n]), w.debug("Added attribute ", r[n].attributeName);
};
var Vt = function(t, r, e, n) {
  ...
  lt.push(s), w.debug("Added new relationship :", s);
};
var Ut = () => lt;
var Ht = function() {
  j = {}, lt = [], Fl();
...
var oe = (t, r, e) => {
  ...
  let c = false, _ = false, E = 0, g = 0, O = 0, y = 0, A = f.height + n * 2, Y = 1;
  e.forEach((N) => {
    ...
    let k = 0;
    const x = Dd(N.attributeType), P = t.append("text").classed("er entityLabel", true).attr("id", `${v}-type`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", Ft().fontFamily).style("font-size", p + "px").text(x), Z = t.append("text").classed("er entityLabel", true).attr("id", `${v}-name`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", Ft().fontFamily).style("font-size", p + "px").text(N.attributeName), w2 = {};
    w2.tn = P, w2.nn = Z;
    const H = P.node().getBBox(), S2 = Z.node().getBBox();
    if (E = Math.max(E, H.width), g = Math.max(g, S2.width), k = Math.max(H.height, S2.height), c) {
      const M = N.attributeKeyTypeList !== void 0 ? N.attributeKeyTypeList.join(",") : "", a = t.append("text").classed("er entityLabel", true).attr("id", `${v}-key`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", Ft().fontFamily).style("font-size", p + "px").text(M);
      w2.kn = a;
      const l = a.node().getBBox();
      O = Math.max(O, l.width), k = Math.max(k, l.height);
    }
    if (_) {
      const M = t.append("text").classed("er entityLabel", true).attr("id", `${v}-comment`).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "left").style("font-family", Ft().fontFamily).style("font-size", p + "px").text(N.attributeComment || "");
      w2.cn = M;
      const a = M.node().getBBox();
      y = Math.max(y, a.width), k = Math.max(k, a.height);
    }
    w2.height = k, o.push(w2), A += k + n * 2, Y += 1;
  });
  let U = 4;
  c && (U += 2), _ && (U += 2);
  const X = E + g + O + y, I = {
  ...
  if (e.length > 0) {
    ...
    o.forEach((x) => {
      ...
      const Z = t.insert("rect", "#" + x.tn.node().id).classed(`er ${k}`, true).attr("x", 0).attr("y", v).attr("width", E + s * 2 + N).attr("height", x.height + n * 2), w2 = parseFloat(Z.attr("x")) + parseFloat(Z.attr("width"));
      x.nn.attr(
        "transform",
        "translate(" + (w2 + s) + "," + P + ")"
      );
      const H = t.insert("rect", "#" + x.nn.node().id).classed(`er ${k}`, true).attr("x", w2).attr("y", v).attr("width", g + s * 2 + N).attr("height", x.height + n * 2);
      let S2 = parseFloat(H.attr("x")) + parseFloat(H.attr("width"));
      if (c) {
        x.kn.attr(
          "transform",
          "translate(" + (S2 + s) + "," + P + ")"
        );
        const M = t.insert("rect", "#" + x.kn.node().id).classed(`er ${k}`, true).attr("x", S2).attr("y", v).attr("width", O + s * 2 + N).attr("height", x.height + n * 2);
        S2 = parseFloat(M.attr("x")) + parseFloat(M.attr("width"));
      }
      _ && (x.cn.attr(
        "transform",
        "translate(" + (S2 + s) + "," + P + ")"
      ), t.insert("rect", "#" + x.cn.node().id).classed(`er ${k}`, "true").attr("x", S2).attr("y", v).attr("width", y + s * 2 + N).attr("height", x.height + n * 2)), v += x.height + n * 2, k = k === "attributeBoxOdd" ? "attributeBoxEven" : "attributeBoxOdd";
    });
  } else
...
var ce = function(t, r, e) {
  ...
  return n.forEach(function(p) {
    ...
    s = s === void 0 ? f : s;
    const c = "text-" + f, _ = o.append("text").classed("er entityLabel", true).attr("id", c).attr("x", 0).attr("y", 0).style("dominant-baseline", "middle").style("text-anchor", "middle").style("font-family", Ft().fontFamily).style("font-size", R.fontSize + "px").text(p), { width: E, height: g } = oe(
      o,
      _,
      r[p].attributes
...
var de = function(t, r, e, n, s) {
  ...
  const p = e.edge(
    ...
    return A.y;
  }).curve(Uf), o = t.insert("path", "#" + n).classed("er relationshipLine", true).attr("d", f(p.points)).style("stroke", R.stroke).style("fill", "none");
  ...
  const _ = o.node().getTotalLength(), E = o.node().getPointAtLength(_ * 0.5), g = "rel" + ft, y = t.append("text").classed("er relationshipLabel", true).attr("id", g).attr("x", E.x).attr("y", E.y).style("text-anchor", "middle").style("dominant-baseline", "middle").style("font-family", Ft().fontFamily).style("font-size", R.fontSize + "px").text(r.roleA).node().getBBox();
...
var ue = function(t, r, e, n) {
  R = Ft().er, w.info("Drawing ER diagram");
  const s = Ft().securityLevel;
  let p;
  s === "sandbox" && (p = Ct("#i" + r));
  const o = (s === "sandbox" ? Ct(p.nodes()[0].contentDocument.body) : Ct("body")).select(`[id='${r}']`);
  C.insertMarkers(o, R);
  let c;
  c = new L({
    multigraph: true,
  ...
  js(c), le(o, c), E.forEach(function(Y) {
    de(o, Y, c, _, n);
  });
  const g = R.diagramPadding;
  Di.insertTitle(o, "entityTitleText", R.titleTopMargin, n.db.getDiagramTitle());
  const O = o.node().getBBox(), y = O.width + g * 2, A = O.height + g * 2;
  Tl(o, A, y, R.useMaxWidth), o.attr("viewBox", `${O.x - g} ${O.y - g} ${y} ${A}`);
};
...
//# sourceMappingURL=erDiagram-9aadf2be-JGKNDD3T.js.map
